...
TITEM * createItem (  char i , TITEM * next )  { 
  TITEM * l =  ( TITEM * )  malloc  (  sizeof ( * l )  ) ;  //allocate the memory
  l -> m_Digit = i ; 
  l -> m_Next = next ; 
  return l ; 
} 
...
//element of array being the address of the pointer to the the linkd list
...
/*function which returns a different digit depending on the results from comparinh the two linked list*/
int compareLists(const TITEM* list1, const TITEM* list2)
{
    int size1 = 0, size2 = 0;
    TITEM* temp1 = list1;
    TITEM* temp2 = list2;

/*in.vali (checking every elements of the list is ne digit character)*/
/*compare the size of the linnked list*/

    while (temp1 != NULL){
        if(!isdigit(temp1->m_Digit)){  
            return 3;
        }
        size1++;
        temp1 = temp1->m_Next; //  temp1.m_Next being the pointer as wells
    }

    while (temp2 != NULL){
        size2++;
        if(!isdigit(temp2->m_Digit)){  
            return 3;
        }
        temp2 = temp2->m_Next;
    }
...
//resetting because them = NULL
    ...
    if(temp1->m_Digit > temp2->m_Digit)
        return 1;
    else if (temp2 ->m_Digit > temp1->m_Digit)
        return -1;
